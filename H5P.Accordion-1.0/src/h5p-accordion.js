 ...
 * @param {jQuery} $
 */
H5P.Accordion = (function ($, Question) {
  ...
  Accordion.prototype.attach = function ($container) {
    ...
    if (self.$content === undefined) {
      ...
      for (var i = 0; i < self.params.panels.length; i++) {
        self.createPanel(i, self.params.panels[i].titleTTS);
      }
   ...
   * @param {number} id
   */
  Accordion.prototype.createPanel = function (id, titleTTS) {
    var self = this;
    ...
    // Show tts title button
    if(self.params.enableTTSButtons) {
        if(typeof titleTTS !== "undefined") {
          H5P.Question.prototype.addTTSButton(titleTTS, "prependTo", $title);
        }
    }
