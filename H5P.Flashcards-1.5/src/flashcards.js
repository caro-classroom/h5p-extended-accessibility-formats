 ...
 * @param {H5P.jQuery} $
 */
H5P.Flashcards = (function ($, XapiGenerator, Question) {
  ...
  C.prototype.cardsLoaded = function () {
    ...
    if(this.options.behaviour.enableTTSButtons && this.options.introductionTTS !== undefined) {
      H5P.Question.prototype.addTTSButton(this.options.introductionTTS, "prependTo", $(".h5p-description"));
    }
  ...
  C.prototype.addCard = function (index, $inner) {
    ...
    var $anwerButton = this.options.behaviour.disableButtons ? 
      "" :
    ...
    if(that.options.behaviour.enableTTSButtons) {
      H5P.Question.prototype.addTTSButton(card.tts, "prependTo", $card.find('.h5p-imagetext'));
    }
  ...
  return C;
})(H5P.jQuery, H5P.Flashcards.xapiGenerator, H5P.Question);
